    ...
    $categories = [
        'Food' => ['amazon', 'starbucks', 'mcdonalds', 'burgerking', 'kfc', 'subway', 'grocery', 'carrefour'],
    ...
    foreach ($transactions as $trans) {
        ...
        $storeName = '';
        if (preg_match('/to\s+(.+?)\s+\[/', $desc, $matches)) {
            $storeName = $matches[1];
        }

        if ($storeName) {
            if (!isset($topSpending[$matchedCategory][$storeName])) $topSpending[$matchedCategory][$storeName] = 0;
            $topSpending[$matchedCategory][$storeName] += $amount;
        }

        $stmt2 = $pdo->prepare("SELECT user_id FROM ACCOUNTS WHERE id=?");
        $stmt2->execute([$accountId]);
        $user = $stmt2->fetch();
        if (!$user) continue;
        $userId = $user['user_id'];

        if (!isset($userCategorySpending[$userId][$matchedCategory])) $userCategorySpending[$userId][$matchedCategory] = 0;
        $userCategorySpending[$userId][$matchedCategory] += $amount;
    }
    ...
    $usersOverBudget = [];
    foreach ($userCategorySpending as $userId => $categoriesSpent) {
        foreach ($categoriesSpent as $cat => $spent) {
    ...
    <body>
        ...
        <div class="dashboard-container">
            <h2>Top 3 Stores per Category</h2>
